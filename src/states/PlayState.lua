...
function PlayState:init()
    ...
    -- No more matches label off-screen
    self.NoMatchesLabel = -64
...
function PlayState:update(dt)
    ...
    if self.board:countMatches() == 0 then
        self.canInput = false
        Timer.tween(0.25, {
            [self] = {NoMatchesLabel = VIRTUAL_HEIGHT / 2 - 8}
        })
        
        :finish(function()
            Timer.after(1, function()
                Timer.tween(0.25, {
                    [self] = {NoMatchesLabel = VIRTUAL_HEIGHT + 30}
                })
                :finish(function()
                    while self.board:countMatches() == 0 do
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            self.score = self.score + #match * 50 + self.board.varietySum * 50
...
function PlayState:render()
    ...
    love.graphics.rectangle('fill', 16, 16, 186, 136, 4)
    ...
    love.graphics.printf('Matches left: ' .. tostring(self.board:countMatches()), 20, 136, 182, 'center')

    if not self.canInput then
        love.graphics.setColor(95/255, 205/255, 228/255, 200/255)
        love.graphics.rectangle('fill', 0, self.NoMatchesLabel - 8, VIRTUAL_WIDTH, 48)
        love.graphics.setColor(1, 1, 1, 1)
        love.graphics.setFont(gFonts['medium'])
        love.graphics.printf('No more Matches, restart board',
        0, self.NoMatchesLabel, VIRTUAL_WIDTH, 'center')
    end
end

--function PlayState:swapTI(tile1, tile2)
--    -- swap grid positions of tiles
--    tempX = tile1.gridX
--    tempY = tile1.gridY
--
